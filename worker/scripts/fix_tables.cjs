...
console.log("Fetching questions with markdown tables...");
const output = execSync('npx wrangler d1 execute preexam --remote --command="SELECT id, question_text FROM questions WHERE id IN (376, 377, 378, 379, 380)"', { encoding: 'utf-8' });

try {
    const jsonStr = output.substring(output.indexOf('['));
    const data = JSON.parse(jsonStr)[0].results;
    console.log(`Found ${data.length} questions to fix.`);
...
const fs = require('fs');
    ...
    for (let i = 0; i < data.length; i++) {
        const q = data[i];
        const parts = q.question_text.split('<br /><br />');
        if (parts.length >= 2) {
            const actualQuestion = parts.slice(1).join('<br /><br />');
            const newText = `จากข้อมูลในตาราง${htmlTable.replace(/\n/g, '')}<br /><br /><div class="font-bold text-lg mt-4">${actualQuestion}</div>`;
            const escapedText = newText.replace(/'/g, "''");
            sqlContent += `UPDATE questions SET question_text = '${escapedText}' WHERE id = ${q.id};\n`;
        }
    }
    fs.writeFileSync('update.sql', sqlContent, 'utf-8');
    console.log("SQL file generated. Running it...");
